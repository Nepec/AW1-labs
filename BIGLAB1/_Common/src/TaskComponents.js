  ...
  return (
    <ListGroup.Item className="d-flex align-items-center">
      ...
      <Col as="span" className="text-dark text-center">
        {props.isPrivate && <PeopleFill />}
      </Col>
      <Col as="span" className="font-075 text-right d-flex justify-content-end">
    ...
    <Dropdown className="d-inline ml-2">
      ...
      <Dropdown.Menu className="min-width-5" as={CustomMenu}>
        <Dropdown.Item
          className="px-3"
        >
          <PencilSquare className="mr-3" size={16} />Edit
                </Dropdown.Item>
        <Dropdown.Item
          className="px-3"
          onClick={() => props.deleteTask(props.taskID)}
        >
          <Trash2Fill className="mr-3" size={16} />Delete
                </Dropdown.Item>
      </Dropdown.Menu>
    </Dropdown>
  );
...
function TaskList(props) {
  let newList = applyFilter(props.activeFilter, props.tl);

  const deleteTask = (tskID) => {
    props.deleteTask(tsks => tsks.filter(t => t.id !== tskID));
  }
